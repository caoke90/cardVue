...
<div class="card-main">
  <div class="m-img-box">
    <ul
     ...
     :style="{width:pageWidth+'px', height:pageHeight+'px'}">
      <li
       v-for="item in picsArr"
       :style="{transform:translateX(item), transitionDuration:transitionDuration(item)}"
       :class="{'cur':item.cur}"
       @transitionend="initPic"
       @click="onClickPic($event, item)">
        <img :src="item[imgUrlKey]">
        <div class="text-bar" v-if="titleKey && item[titleKey]">
          <span
           class="m-text-cut"
           :style="{ paddingRight: dotPadding + 'px' }"
           v-text="item[titleKey]"></span>
        </div>
      </li>
    </ul>
  </div>
  <ul
   ref="dot"
   class="m-sld-dot"
   v-if="picsArr.length > 1 && dotVal">
    <li
     :class="{'cur':index === pageIndex}"
     v-for="(item, index) in picsArr"></li>
  </ul>
</div>
</template>
